DROP VIEW IF EXISTS _wp_posts;
DROP VIEW IF EXISTS _wp_media;

CREATE VIEW _wp_posts
AS
SELECT
  CASE
    when i.type = 'post' then b.url || '/?p=' || i.ID
    when i.type = 'page' then b.url || '/?page_id=' || i.ID
    else b.url
  END _URL,
  b.url || '/wp-admin/post.php?post=' || i.ID || '&action=edit' _ADMIN,
  b.url || '?rest_route=/wp/v2/' || i.type || 's/' || i.ID URL_WPJSON,
  i.*
FROM
...
SELECT
  ...
  END _URL,
  b.url || '/wp-admin/post.php?post=' || i.ID || '&action=edit' _ADMIN,
  b.url || '?rest_route=/wp/v2/media/' || i.ID URL_WPJSON,
 i.*
FROM
